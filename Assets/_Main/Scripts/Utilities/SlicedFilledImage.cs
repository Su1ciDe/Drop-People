...
#pragma warning restore 1692
	...
	private void GenerateSlicedFilledSprite(VertexHelper vh)
	{
		...
		if (!HasBorder)
		{
			...
			Vector4 vertices = new Vector4(rect.x + rect.width * (padding.x / spriteW), rect.y + rect.height * (padding.y / spriteH), rect.x + rect.width * ((spriteW - padding.z) / spriteW),
		...
		if (m_FillDirection == FillDirection.Left || m_FillDirection == FillDirection.Right)
		{
			...
			var totalSize = rect.width - padding.x - padding.z;
		...
		else
		{
			...
			var totalSize = rect.height - padding.y - padding.w;
			_1OverTotalSize = totalSize > 0f ? 1f / totalSize : 1f;
		}
	...
	private Vector4 GetAdjustedBorders(Vector4 border, Rect adjustedRect)
	{
		...
		for (int axis = 0; axis <= 1; axis++)
		{
			...
			// If the rect is smaller than the combined borders, then there is no room for the borders at their normal size.
			// To avoid artifacts with overlapping borders, we scale the borders down to fit.
	...
	bool ICanvasRaycastFilter.IsRaycastLocationValid(Vector2 screenPoint, Camera eventCamera)
	{
		...
		// Convert to have a lower left corner as a reference point.
		...
		var x = Mathf.Lerp(textureRect.x, textureRect.xMax, normalized.x) / activeSprite.texture.width;
		var y = Mathf.Lerp(textureRect.y, textureRect.yMax, normalized.y) / activeSprite.texture.height;
		...
		{
			Debug.LogError(
				"Using alphaHitTestMinimumThreshold greater than 0 on Image whose sprite texture cannot be read. " + e.Message + " Also make sure to disable sprite packing for this sprite.", this);
			return true;
		}
	}
	...
	{
		if (activeSprite && activeSprite.texture is null)
		{
#if UNITY_2017_4 || UNITY_2018_2_OR_NEWER
			if (!s_Initialized)
			{
